...
# Set the working directory to the location of your dataset if needed
# setwd("path/to/your/directory")

# Read the dataset into an R data frame
customer_data <- read_csv("customer_data.csv")

# Recode purchase frequency as an ordered factor to preserve order in statistical tests
customer_data$purchase_frequency <- factor(customer_data$purchase_frequency, 
                                           levels = c("rare", "occasional", "frequent"),
                                           ordered = TRUE)

# Convert ordered factors to numeric for correlation analysis
customer_data$purchase_frequency_numeric <- as.numeric(customer_data$purchase_frequency)

# Perform a correlation test and extract the correlation coefficient
correlation_test <- cor.test(customer_data$income, customer_data$purchase_frequency_numeric, 
                             method = "spearman")  # Spearman's rank correlation, more appropriate for ordinal data
correlation <- correlation_test$estimate

# Plotting the data using ggplot2
ggplot(customer_data, aes(x = income, y = purchase_frequency_numeric)) +
  geom_point(alpha = 0.5) +
  geom_smooth(method = "lm", color = "red") +
  labs(title = paste("Income vs. Purchase Frequency Correlation: Spearman rho =", round(correlation, 2)),
       x = "Income",
       y = "Purchase Frequency (Numeric)") +
  theme_minimal()
...
# Visualizing the distribution of satisfaction scores
ggplot(customer_data, aes(x = satisfaction_score)) +
  geom_histogram(binwidth = 1, fill = "blue", color = "black") +
  labs(title = "Distribution of Satisfaction Scores", x = "Satisfaction Score", y = "Count") +
  theme_minimal()

# Visualizing the relationship between satisfaction scores and purchase frequencies
ggplot(customer_data, aes(x = satisfaction_score, y = as.numeric(purchase_frequency), group = purchase_frequency)) +
  geom_boxplot(aes(fill = purchase_frequency)) +
  labs(title = "Purchase Frequency by Satisfaction Score", x = "Satisfaction Score", y = "Purchase Frequency (Numeric)") +
  theme_minimal() +
  scale_fill_brewer(palette = "Pastel1")  # Color coding for different purchase frequencies

# Statistical Analysis: Correlation or Chi-Square Test depending on the data structure
# If satisfaction score is treated as numeric
cor_test <- cor.test(customer_data$satisfaction_score, as.numeric(customer_data$purchase_frequency), method = "spearman")
print(cor_test)

# If satisfaction score is treated as categorical
# Convert to a factor if not already
customer_data$satisfaction_score <- factor(customer_data$satisfaction_score)
# Chi-square test
chi_sq_test <- chisq.test(table(customer_data$satisfaction_score, customer_data$purchase_frequency))
print(chi_sq_test)
...
data$purchase_frequency <- factor(data$purchase_frequency, levels = c("rare", "occasional", "frequent"))
data$income_level <- ifelse(data$income > median(data$income, na.rm = TRUE), "High", "Low")
...
income_purchase_cor <- cor(data$income, data$purchase_amount, use = "complete.obs")

# Linear regression for satisfaction score influence
lm_result <- lm(purchase_amount ~ satisfaction_score, data = data)

# Multiple regression for demographic factors
multi_lm <- lm(purchase_amount ~ age + gender + income + education, data = data)

# Interaction effect of education and income level
interaction_lm <- lm(purchase_amount ~ education * income_level, data = data)

# Visualization of income distribution and purchase amount by frequency
p1 <- ggplot(data, aes(x = income)) + geom_histogram(bins = 30, fill = "blue") + ggtitle("Income Distribution")

p2 <- ggplot(data, aes(x = purchase_frequency, y = purchase_amount)) + geom_boxplot() + ggtitle("Purchase Amount by Frequency")
